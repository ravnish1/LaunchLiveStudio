...
export const CTABanner = ({ compact = false }: { compact?: boolean }) => {
  ...
  return (
    <section className={`${compact ? 'py-12' : 'py-12 md:py-20'} px-4 md:px-6 overflow-hidden`}>
      <div className={`${compact ? 'max-w-[1000px]' : 'max-w-[960px]'} mx-auto w-full`}>
          <div className={`relative rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden bg-accent ${compact ? 'p-6 md:p-10' : 'p-5 sm:p-8 md:p-14'} text-center group`}>
            ...
            <div className={`relative z-10 ${compact ? 'space-y-6 md:space-y-8' : 'space-y-6 md:space-y-10'}`}>
              ...
              <p className={`text-white/90 max-w-xl mx-auto font-medium leading-relaxed px-2 md:px-0 ${compact ? 'text-sm sm:text-base md:text-lg' : 'text-sm sm:text-base md:text-lg'}`}>
                Quality over volume. Reach out — we value your time and reply within 2 hours.
              </p>
              
              <div className="flex flex-col sm:flex-row justify-center items-center gap-3 w-full pt-2 md:pt-4">
                <button 
                  onClick={() => router.push('/book-a-call')} 
                  className={`w-full sm:w-auto px-6 md:px-8 py-3 md:py-3.5 bg-white text-accent font-bold rounded-full hover:scale-105 active:scale-95 transition-all shadow-xl shadow-black/20 text-center ${compact ? 'text-sm md:text-base' : 'text-sm md:text-base'}`}
                >
                  Start a Project &rarr;
                </button>
                <a
                  href="mailto:hello@launchlive.studio"
                  className={`w-full sm:w-auto px-6 md:px-8 py-3 md:py-3.5 bg-white/10 hover:bg-white/20 text-white font-bold rounded-full transition-all border border-white/10 text-center whitespace-nowrap ${compact ? 'text-xs md:text-sm' : 'text-xs md:text-sm'}`}
                >
